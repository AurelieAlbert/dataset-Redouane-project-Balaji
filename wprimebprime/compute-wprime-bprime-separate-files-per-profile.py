...
#
"""
This script extracts profiles of temperature, salinity and vertical velocity in 1degx1deg boxes and one date in eNATL60-BLBT02. It computes buoyancy and spatially filters it and the vertical velocity. The resulting product of the to quantities (w' and b') are saved in a single file
"""
...
def compute_mean_wprime_bprime(date,ibox,box,imin,imax,jmin,jmax,box_name):
    print('read the data')
    ...
    profile=wprimebprime[:,:,jmin[ibox]:jmax[ibox],imin[ibox]:imax[ibox]]
    print('mkdir output path')
    path_out='/scratch/cnt0024/hmg2840/albert7a/eNATL60/eNATL60-BLBT02-S/RL/'+str(box)+'/'+str(ibox)
    try:
      os.mkdir(path_out) 
    except OSError as error: 
      print(error)
    print('to dataset')
    for tt in np.arange(25):
      for yt in np.arange(jmax[ibox]-jmin[ibox]):
        for xt in np.arange(imax[ibox]-imin[ibox]):
          dataset=profile[tt,:,yt,xt].to_dataset(name='wprimebprime')
          ...
          dataset.attrs['global_attribute']= 'vertical flux of small scales buoyancy times w profile '+box_name[ibox]+' computed on occigen '+str(today)
          profile_name='/scratch/cnt0024/hmg2840/albert7a/eNATL60/eNATL60-BLBT02-S/RL/'+str(box)+'/'+str(ibox)+'/eNATL60'+str(box)+box_name[ibox]+'t'+str(tt)+'y'+str(yt)+'x'+str(xt)+'-BLBT02_y'+date[0:4]+'m'+date[4:6]+'d'+date[6:9]+'_wprimebprime-profile.nc'
          if not os.path.exists(profile_name):
            print("Writing wprimebprime profile for tt,yt,xt = "+str(tt)+', '+str(yt)+', '+str(xt))
            dataset.to_netcdf(path=profile_name,mode='w')
          else:
            print(" wprimebprime profile for tt,yt,xt = "+str(tt)+', '+str(yt)+', '+str(xt)+' already written')
...
def main():
    ...
    compute_mean_wprime_bprime(date,k,box,imin,imax,jmin,jmax,box_name)
